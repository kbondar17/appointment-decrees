...
class LinksGetter:
    ...
    def __init__(self, logger) -> None:
        self.regions_n_number = json.load(open(Path(__file__).parent / 'regions_n_their_numbers.json'))
    ...
    def create_url(self)->str:
        """подставляем параметры (DATE, SEARCH_WORD, REGION ) в url"""
        try:
            reg_n = self.regions_n_number[REGION]
        except KeyError:
            raise KeyError('Неверно указано название региона. Список регионов в regions.txt')

        search_word = urllib.parse.quote(SEARCH_WORD, encoding='cp1251')

        url_for_getting_files = f'''http://pravo.gov.ru/proxy/ips/?searchlist=&bpas={reg_n}&a3=&a3type=1&a3value=&a6=&a6type=1&a6value=&a15=&a15type=1&a15value=&a7type=3&a7from=&a7to=&a7date={DATE}&a8=&a8type=1&a1=&a0={search_word}&a16=&a16type=1&a16value=&a17=&a17type=1&a17value=&a4=&a4type=1&a4value=&a23=&a23type=1&a23value=&textpres=&sort=7&x=48&y=8&lstsize=200&start=0'''

        return url_for_getting_files
    ...
    def filter_and_get_authors(self, doc_id:str, tag:str)->tuple|None:
    ...
    @BackOff()
    @Log(__name__)
    def get_links(self, tag_we_need:str='назначение') -> dict[str,str]:
    ...
    def download_links(self, destination_path:Path, tag_we_need:str='назначение')->list[str]:
        links_with_info = self.get_links(tag_we_need=tag_we_need)
        with open(destination_path, 'w') as f:
            json.dump(links_with_info, f)
